...
///////////////////////password //////////////////////////////////
  ...
  if(value==0){//////passowrd not set
   ...
   LCD.setCursor(0,2);
   LCD.print("SSID :"+readeeprom(20));
   LCD.setCursor(0,3);
   LCD.print("PASS :"+readeeprom(40));
...
void loop() {
  ...
  if(!attendanceflag&!enrollflag){/////////////start if attendance upload
 ...
 // Serial.println("You're connected to the network");
  ...
  }else if(enrollflag){/////if the enroll flag already set just go the current class

         LCD.clear();
         LCD.setCursor(0,0);//set the cursor
         LCD.print("Enrolling "+classnum);
...
///////////////downloading id complete
    ...
    }else if(attendanceflag){//////if in middle of attendance go to that class again
      
               LCD.clear();
               LCD.setCursor(0,0);
               LCD.print("Attendance "+classnum);
...
{ 
  ...
  Serial.println("\ndone.");
    ...
    // Make a HTTP request
    client.print("GET /registration/info.php?class=");
...
void httpRequest()
{
  ...
  if (client.connect(server, 80)) {
    ...
    client.print("GET /registration/try.php?class=");
...
void httpRequestdown(int num)
{
  ...
  if (client.connect(server, 80)) {
    ...
    // send the HTTP PUT request
    client.print("GET /registration/tryjson.php?class=");
...
void httpReq(int stid)
{
  ...
  if (client.connect(server, 80)) {
    ...
    // send the HTTP PUT request
    client.print("GET /registration/att.php?id=");
    client.print(stid); 
    client.println(" HTTP/1.0");
    client.print("Host: ");
    client.println(server);
